...
| --- | --- | --- |
| `threejs-fundamentals` | `/gamedev-threejs:threejs-fundamentals` | Scene, camera, renderer, Object3D hierarchy |
| `threejs-geometry` | `/gamedev-threejs:threejs-geometry` | Built-in shapes, BufferGeometry, instancing |
| `threejs-materials` | `/gamedev-threejs:threejs-materials` | PBR, standard/phong materials, shaders |
| `threejs-lighting` | `/gamedev-threejs:threejs-lighting` | Light types, shadows, environment lighting |
| `threejs-textures` | `/gamedev-threejs:threejs-textures` | Texture types, UV mapping, render targets |
| `threejs-animation` | `/gamedev-threejs:threejs-animation` | Keyframe, skeletal, morph target animation |
| `threejs-loaders` | `/gamedev-threejs:threejs-loaders` | GLTF/GLB, async loading, caching |
| `threejs-shaders` | `/gamedev-threejs:threejs-shaders` | GLSL, ShaderMaterial, custom effects |
| `threejs-postprocessing` | `/gamedev-threejs:threejs-postprocessing` | EffectComposer, bloom, DOF, custom passes |
| `threejs-interaction` | `/gamedev-threejs:threejs-interaction` | Raycasting, controls, user input |
| `webgpu-threejs-tsl` | `/gamedev-threejs:webgpu-threejs-tsl` | WebGPU renderer, TSL node materials, compute shaders ([dgreenheck/webgpu-claude-skill](https://github.com/dgreenheck/webgpu-claude-skill)) |
...
| `threejs-debug-profiler` | `/gamedev-threejs:threejs-debug-profiler` | Black screens, runtime/loading/resize/mobile bugs, draw calls, triangles, memory, perf |
| `threejs-qa-release` | `/gamedev-threejs:threejs-qa-release` | Production builds, browser/mobile verification, canvas-pixel checks, release risk reports |
| `threejs-3d-generator` | `/gamedev-threejs:threejs-3d-generator` | Tripo API text/image→3D, game-ready GLB/FBX, rigging, animation, conversion (needs `TRIPO_API_KEY`) |
